...
export function useBlockActions(state: any) {
  const { blocks, setBlocks, setSelectedBlock, setSelectedCell, updateCurrentPageBlocks } = state;
  ...
  const updateBlock = (id: string, updated: Partial<Block>) => {
    ...
    if (!targetBlock.editable && targetBlock.locked === false) return;
  ...
  const selectBlock = (id: string | null) => {
    ...
    // テンプレート由来かつ editable:false の場合は選択不可
  ...
  const setAllBlocks = (newBlocks: Block[]) => {
    if (newBlocks.some((b) => b.locked)) return;
    setBlocks(newBlocks);
    setSelectedBlock(null);
    setSelectedCell(null);
  };
