...
export function usePageActions(state: any) {
  ...
  const addPage = () => {
    ...
    const templateBlocks = firstPage?.blocks ? JSON.parse(JSON.stringify(firstPage.blocks)).filter((block: Block) => block.locked === true && block.source === "template") : [];
    const newPage: Page = {
      id: nanoid(),
  ...
  // 現在ページのブロックを更新
  const updateCurrentPageBlocks = (newBlocks: Block[]) => {
    setPages((prev: Page[]) => prev.map((p: Page) => p.number === currentPage ? { ...p, blocks: [...newBlocks] } : p));
    setTimeout(() => state.saveToHistory(), 0);
  };
