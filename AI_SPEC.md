1. プロダクト概要（最終統合版）
本プロダクトは、企業内で扱われる業務文書を 作成・管理・承認・検索・活用 するための統合ドキュメントプラットフォームである。テンプレートベースの文書作成、厳密な承認フロー、フォルダ階層に基づく管理番号体系、全文検索、履歴管理、AI 支援などを組み合わせ、製造業・品質管理・業務標準化領域に適した 高精度かつ監査性の高い文書運用 を実現する。
旧 LocalStorage ベースの文書管理システムは完全に廃止され、現在は Prisma + SQLite を基盤としたサーバーサイド文書管理システム に統合されている。これにより、データの永続性・整合性・検索性・承認フローの厳密性が大幅に向上した。
...
- admin：すべての操作が可能
※creator ロールは 3章（User モデル）・5章（承認フロー）・11章（権限モデル）と整合するよう正式に定義。
...
7. 検索・全文検索・AI インデックス仕様［最終統合版］
検索機能は文書管理システムの中心機能であり、
メタデータ検索・全文検索・ブロック検索・AI インデックス検索（RAG） の 4 層で構成される。
さらに、検索結果をもとに 文書プレビュー を行い、
上位文書を LLM に渡して AI 生成 を行う仕組みを備える。

7.1 検索の 4 レイヤー構造
検索は次の 4 つのレイヤーで構成される。
- メタデータ検索（title, management_number, folder, type）
- 全文検索（fullText）
- ブロック検索（content.blocks）
- AI インデックス検索（semantic search / embedding）
これらを組み合わせて高精度な検索を実現する。

7.2 メタデータ検索
検索対象となるメタ情報は次の通り。
- 文書番号（management_number）
- タイトル（title）
- 文書種別（document_type.code）
- フォルダ階層（folder.code）
- 作成者（creator.name）
- ステータス（status）
- 作成日・更新日
メタデータ検索は高速で、最初のフィルタリングに使用される。

7.3 全文検索（fullText）
全文検索は Document.fullText を対象とする。
fullText は Document.content.blocks から抽出したテキストを連結したもので、
次の要素を含む。
- text ブロックの label
- table セルの text
- title / subtitle の value
画像・図形などテキストを持たないブロックは対象外。

7.4 ブロック検索（content.blocks）
全文検索では拾えない構造的な検索を行うため、
content.blocks を直接検索対象にする。
例
- 「画像ブロックを含む文書」
- 「赤色の図形がある文書」
- 「表のセルに '検査' が含まれる文書」

7.5 AI インデックス（RAG）検索
AI インデックスは、意味検索（semantic search）を実現するための仕組みである。
インデックス対象（最終統一仕様）
- fullText のみ
対象外（明確化）
- blocks のテキスト
- RevisionHistory のテキスト
- メタ情報（folder.code, document_type など）
※以前の仕様では blocks や RevisionHistory も対象とする記述があったが、
AI インデックス対象は fullText のみに統一する。
...
7.6 AI インデックスの更新タイミング
AI インデックスは fullText の更新時に再生成される。
更新タイミング：
- 文書作成（submit）
- 文書更新（draft 保存）
- 改訂（revision）
...
7.7 検索結果のプレビュー（管理番号提示）
検索結果から文書を選択すると、
管理番号（management_number）をキーとして文書をプレビューできる。
プレビュー内容
- 文書番号
- タイトル
- 文書種別
...
- 承認履歴（1ページ目の表）
- 本文（blocks をレンダリング）
プレビューは Document.content.blocks をそのまま描画する。
（8章のブロック仕様と統一）

7.8 AI 生成の入力データ（検索上位 N 件）
AI 生成は、検索結果の 上位 N 件（例：3〜5 件） を LLM に渡して行う。
入力データ
- 文書番号（management_number）
- タイトル
- 抜粋（snippet）
...
- 文書の差分説明
重要仕様
- AI に渡すデータは常に検索結果の上位 N 件のみ
- 全データを渡すことはしない（偽データ混入防止）

7.9 偽データ対策：会話リセット（新規会話）
AI 生成は LLM の会話履歴に依存するため、
偽データ（hallucination）が蓄積するリスクがある。
会話リセット仕様
- 「新規会話」ボタンで LLM のコンテキストをリセット
- リセット後は検索結果の上位 N 件のみを再ロード
- 過去の生成内容は引き継がない
- 文書番号を指定した場合は、その文書のみを再ロード
リセットが必要なケース
- AI が誤った文書内容を引用し始めた
- 過去の会話がノイズになっている
- 新しい文書を基準に生成したい
...
7.10 検索とアクセス権の関係
検索結果は user_folder_access に基づきフィルタされる。
- canRead = true のフォルダのみ検索対象
- admin はすべての文書を検索可能
AI 生成も同様にアクセス権を尊重する。

7.11 検索 → プレビュー → AI 生成の流れ（テキスト形式）
- ユーザーがキーワードを入力
- メタデータ検索・全文検索・AI 検索を統合して結果を返す
- 文書番号を提示し、プレビュー可能
- 上位 N 件を LLM に渡して AI 生成
- 必要に応じて会話リセットでコンテキストを初期化
